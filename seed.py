...
# Целевые объёмы для расширенного сидинга
TARGET_USERS = 100
TARGET_BOOKINGS = 500

# Placeholder-фото (picsum выдаёт стабильные изображения по seed-у)
def _placeholder_photo(seed: int) -> str:
    return f"https://picsum.photos/seed/workspot{seed}/400/300"
...
async def seed_users(db: AsyncSession) -> list[User]:
    """Создаёт фиксированных + сгенерированных пользователей до TARGET_USERS."""
    created: list[User] = []
    for data in _USERS:
    ...
    # Догоняем до TARGET_USERS: bulk-generated test users
    shared_hash = _hash("password123")
    existing_emails_res = await db.execute(select(User.email))
    existing_emails = {row[0] for row in existing_emails_res.all()}

    idx = 1
    while len(created) < TARGET_USERS:
        email = f"bulk{idx}@workspot.ru"
        idx += 1
        ...
        user = User(
            ...
            name=f"Тестовый пользователь #{idx}",
            role=UserRole.user,
        )
        db.add(user)
        created.append(user)
        existing_emails.add(email)
        if idx > TARGET_USERS * 10:  # safety stop
            break
...
async def seed_spaces(db: AsyncSession) -> list[Space]:
    ...
    created: list[Space] = []
    for i, data in enumerate(_SPACES, start=1):
        photos = list(data["photos"]) + [_placeholder_photo(i)]
...
async def seed_bookings(
    ...
    """
    Создаёт до TARGET_BOOKINGS активных броней по пользователям с ролью user.
    ...
    # Сколько ещё активных броней нужно дозалить?
    res = await db.execute(select(Booking).where(Booking.status == BookingStatus.active))
    already = len(res.scalars().all())
    remaining = max(0, TARGET_BOOKINGS - already)
    if remaining == 0:
        return []

    random.shuffle(free_slots)
    slots_to_book = free_slots[: min(remaining, len(free_slots))]
...
async def seed_minio_bucket() -> None:
    """Создаёт bucket в MinIO, если задан env MINIO_URL.

    Тихо игнорирует ошибки соединения — сидер должен работать и без MinIO.
    """
    ...
    try:
        ...
        print(f"   MinIO bucket:  {client.bucket} (ensured) @ {minio_url}")
    except Exception as exc:  # pragma: no cover
        print(f"   MinIO bucket:  пропущено ({type(exc).__name__}: {exc})")
